...
1. Use `git` to retrieve [JMES Core](https://github.com/jmesworld/core/), which contains the latest stable release. You can find the latest tag on the [tags page](https://github.com/jmesworld/core/tags) or via autocomplete in your terminal: type `git checkout v` and press `<TAB>`.

   ```
   git clone https://github.com/jmesworld/core
   ```

The specific branch `release/0.47.x-testnet` will be the branch to checkout instead to directly have everything setup for the testnet.   

  ```
   git clone -b release/0.47.x-testnet https://github.com/jmesworld/core
   ```
